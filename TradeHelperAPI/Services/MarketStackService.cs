...
namespace TradeHelper.Services
{
    ...
    public class MarketStackService
    {
        ...
        private async Task<List<OhlcTechnicalCalculator.OhlcBar>?> FetchEodOhlcAsync(string symbol, int days = 260)
        {
            ...
            try
            {
                ...
                var bars = new List<OhlcTechnicalCalculator.OhlcBar>();
                foreach (var item in data.EnumerateArray())
                {
                    if (item.TryGetProperty("open", out var o) && o.TryGetDouble(out var open) &&
                        ...
                        bars.Add(new OhlcTechnicalCalculator.OhlcBar(open, high, low, close));
                }
                doc.Dispose();
        ...
        public async Task<Dictionary<string, double>> FetchTechnicalIndicatorsAsync(string instrumentName)
        {
            ...
            foreach (var kv in OhlcTechnicalCalculator.Compute(bars))
